...
function MatchDragDrop({ text }) {
    const originalPairs = decompressText(text).split('\n\n').map(pair => {
    ...
    const [definitions] = useState(shuffled(originalPairs.map((p, idx) => ({ ...p, id: idx }))));
    const [matchedPairs, setMatchedPairs] = useState([]);

    const [showCelebration, setShowCelebration] = useState(false);

    const dropHandler = (termIndex, defIndex) => {
        const definitionDiv = document.querySelector(`.droppable[data-id="${defIndex}"]`);
        if (terms[termIndex] === definitions[defIndex].term) {
            ...
            setTimeout(() => {
                const termToHide = document.querySelector(`.draggable[data-id="${termIndex}"]`);
